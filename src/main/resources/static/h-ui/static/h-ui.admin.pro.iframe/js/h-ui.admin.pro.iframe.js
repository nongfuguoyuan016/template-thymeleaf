...
*/
var num=0,hide_nav=$("#Hui-admin-tabNav");
...
function Hui_admin_tab(obj){
  var bStop = false,
    ...
    thirdMenuName = $(obj).attr("data-third"),
  ...
  if(!bStop){
    creatIframe(href,title,firstMenuName,secondMenuName,thirdMenuName);
    min_titleList();
  }
  else{
...
/*创建iframe*/
function creatIframe(href,titleName,firstMenuName,secondMenuName,thirdMenuName){
  var topWindow=$(window.parent.document),
    show_nav=topWindow.find('#min_title_list'),
    iframe_box=topWindow.find('#iframe_box'),
  ...
  var data = '';
  if(firstMenuName) {
      data += ' data-first=' + firstMenuName;
      if(secondMenuName) {
          data += ' data-second=' + secondMenuName;
          if(thirdMenuName) {
              data += ' data-third=' + thirdMenuName;
          }
      }
  }
  iframe_box.append('<div class="show_iframe"><div class="loading"></div><iframe data-scrollTop="0" frameborder="0" src='+href + data +'></iframe></div>');
...
/**
 * 关闭当前所在的iframe
 * @param href window.location.pathname
 */
function closeCurrIframeByHref(href) {
    $(window.parent.document).find('#min_title_list span[data-href="'+href+'"]').siblings('i').trigger('click')
}
...
$(function(){
  ...
  $(document).on("click","#min_title_list li",function(){
    ...
    if(iframe_box.find(".show_iframe").length > 1) {
  ...
  $(document).on("click","#min_title_list li i",function(){
    ...
    num==0 ? num=0 : num--;
    $("#min_title_list li").eq(num).addClass("active");
    $('#iframe_box').find(".show_iframe").eq(num).show();
    tabNavallwidth();
  });
  ...
  $('#js-tabNav-next').click(function(){
      var li_length =  $("#min_title_list").find('li').length;
    num==li_length-1 ? li_length-1 : num++;
    toNavPos();
  });
